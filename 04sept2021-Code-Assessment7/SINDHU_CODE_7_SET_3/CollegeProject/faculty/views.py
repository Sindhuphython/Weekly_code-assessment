...
def login_view_check(request):
    try:
        ...
        getfaculty=Faculty.objects.filter(username=username,password=password)
        faculty_serializer=FacultySerializer(getfaculty,many=True)
        print(faculty_serializer.data)
        if(faculty_serializer.data):
            for i in faculty_serializer.data:
                x=i["fcode"]
                y=i["id"]
                print(x)
            request.session['ufcode']=x
