...
def wel_view(request):
    return render(request,'wel.html')
@csrf_exempt
def log_view_check(request):
    try:
...
def update_data_read(request):
    ...
    mydata={'name':getnewname,'admno':getnewadmno,'rollno':getnewrollno,'college':getnewcollege,
            'parentname':getnewparentname,'mobileno':getnewmobileno,'dept':getnewdept,'username':getnewusername,
...
def searchapi(request):
    try:
        ...
        student_serializer=StudentSerializer(getstudent,many=True)
        print(student_serializer.data)
        # return JsonResponse(search_serializer.data,safe=False,status=status.HTTP_200_OK)
